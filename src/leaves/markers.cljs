...
;; the bbox culling the shadow near the leaf tip:
(defn svg-leaf [size color]
  [:div {:style {:transform (transform (* -0.5 size) (- size))}}
   [:svg {:width size :height size
    ...
    [:defs
     [:filter {:id "f1", :x "0", :y "0", :width "200%", :height "200%"}
      [:feOffset {:result "offOut", :in "SourceAlpha", :dx 32, :dy 32}]
      [:feGaussianBlur {:result "blurOut", :in "offOut", :stdDeviation 16}]
      [:feBlend {:in "SourceGraphic", :in2 "blurOut", :mode "normal"}]]]
    [:path
     {:fill color, :filter "url(#f1)",
      :d (str "M356.208,107.051c-1.531-5.738-4.64-11.852-6.94-17.205C321.746,23.704,261.611,0,213.055,0"
